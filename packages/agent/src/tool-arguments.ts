...
export function prepareToolArguments(tool: Pick<AgentTool, "prepareArguments">, args: unknown): unknown {
	if (!tool.prepareArguments) return args;
	return tool.prepareArguments(structuredClone(args));
}
...
export function prepareAgentToolCallArguments(tool: AgentTool, toolCall: AgentToolCall): AgentToolCall {
	...
	return {
		...toolCall,
		arguments: prepareToolArguments(tool, toolCall.arguments) as Record<string, unknown>,
	};
}
