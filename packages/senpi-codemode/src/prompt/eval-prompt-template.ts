...
A cell that outlives the foreground window detaches when detached capacity is available; otherwise it stays foreground until settlement or a kill deadline. Same-language cells queue behind it; another language can continue. A detached cell completes as one notification with its value or error and buffered output. Its own execution time is capped at {{runBudgetSeconds}}s — queue time and time inside host tool calls are not charged; the hard limit stays wall-clock from submission; raise \`timeout\` only for a declared long run — and a killed js cell that cannot settle restarts its kernel with every global lost. Do not re-run a detached cell; read or cancel it with \`eval({ action: "peek", cell_id })\` / \`eval({ action: "stop", cell_id })\`.
