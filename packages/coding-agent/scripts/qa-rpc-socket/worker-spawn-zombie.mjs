 ...
 *
 * Usage:
 *   node --import tsx scripts/qa-rpc-socket/worker-spawn-zombie.mjs \
 *     --runtime in-process [--spawns 50] [--settle-ms 6000] [--max-zombies <n>] [--out <file>]
 */
...
async function main() {
	...
	transcript.push(`runtime=${runtime} hostPid=${process.pid} spawns=${spawns} settleMs=${settleMs}`);
	...
	try {
		...
		transcript.push(`RESULT runtime=${runtime} spawns=${spawns} settleMs=${settleMs} zombies=${zombies}`);
