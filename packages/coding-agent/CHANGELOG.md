...
- Required and speculative compaction no longer wedges when a relay completes the summarization request with a normal stop and zero text. Some OpenAI-completions relays answer an explicitly pinned reasoning effort on a large tool-bearing summarization prompt with an empty 200 stream (observed on cline-backed z-ai/glm-5.3-flash: HTTP 200 carrying only the role prelude, completion=7 tokens), which surfaced as a terminal "Compaction rejected: summarization response contained no text (stopReason: stop)" after burning the whole overflow-retry budget. An empty stop now spends exactly one retry of the same request without the reasoning-effort override before the terminal empty-summary error; persistent emptiness keeps the deterministic fallback contract.
